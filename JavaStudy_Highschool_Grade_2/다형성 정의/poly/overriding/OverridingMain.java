...
public class OverridingMain {
    public static void main(String[] args) {
        ...
        poly.method();  // 메서드 오버라이딩, 오버라이딩 한 메서드가 우선권을 가짐. (엄청 중요한!)
...
1. child 변수는 Child 타입이다.
    - child.value, child.method()를 호출하면 인스턴스의 Child 타입에서 기능을 탖아 실행한다.
...
3. poly 변수는 Parent 타입이다.
    - poly.value, poly.method()를 호출하면 인스턴스의 Parent 타입에서 기능을 찾아 실행한다.
        ...
        - poly.method() : Parent 타입에 있는 method()를 실행할려고 한다.
                          그런데 하위 타입인 Child.method()가 오버라이딩이 되어 있는걸 확인한다.
